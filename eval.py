...
def get_imagenet_dataloader(imagenet_preproc, batch_size=32):
    ...
    dataset = load_dataset("timm/mini-imagenet", data_files={"test": "data/test-0000*-of-00002.parquet"}, split="test", verification_mode="no_checks")
    ...
    dataset.set_transform(lambda examples: {"image": [transform(img) for img in examples["image"]], "label": [torch.tensor(map_imagenet_class_id(idx, dataset)) for idx in examples["label"]]})
